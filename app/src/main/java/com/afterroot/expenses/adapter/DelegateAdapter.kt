...
class DelegateAdapter(callbacks: ItemSelectedCallback) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private var mList = ArrayList<Expense>()
    private var delegateAdapters = SparseArrayCompat<AdapterType>()

    init {
        with(delegateAdapters) {
            put(Expense.TYPE_GROUP, GroupAdapterType(callbacks))
            put(Expense.TYPE_EXPENSE, ExpenseListAdapterType(callbacks))
            put(Expense.TYPE_GROUP_ALT, GroupAltAdapterType(callbacks))
            put(Expense.TYPE_EXPENSE_SUMMARY, ExpenseSummaryAdapterType(callbacks))
        }
    }
